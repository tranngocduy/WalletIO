...
import { duration, updateHistoryRange, updateSocketOldData, updateDateSelectionMax } from '@/html/stringCode/business';

const BASE_INTERVAL = 1000;
const MIN_DATE_GLOBAL = dayjs('2020-01-01', 'YYYY-MM-DD').startOf('second').valueOf();

export const initChartSpot = initConfig => {
  const jsCode = `
  isViewHistory = false;
  ...
  function _initChart() {
    ...
    cursor.lineX.setAll({ strokeWidth: 1, stroke: am5.color(0x00b6f9),  strokeDasharray: [], visible: true });
    cursor.lineY.setAll({ strokeWidth: 1, stroke: am5.color(0x00b6f9),  strokeDasharray: [], visible: true });

    var xRenderer = am5xy.AxisRendererX.new(root, { minGridDistance: 150 });
    xRenderer.labels.template.setAll({ fill: am5.color(0x5c5c5c), fontSize: 12 });
    ...
    rangeLastPrice.get('label').setAll({
      ...
      background: am5.RoundedRectangle.new(root, {
        isMeasured: false,
        fill: am5.color(0x235793),
        fillOpacity: 0.5,
        stroke: am5.color(0x298bee),
        strokeWidth: 1,
        cornerRadiusBL: 20,
        cornerRadiusBR: 20,
        cornerRadiusTL: 20,
